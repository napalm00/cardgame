...
import Cardgame.GUI.GUIAttacker;
...
public class DefaultCombatPhase implements Phase {
    ...
    public void execute(){
        ...
        if (!current_player.getCreatures().isEmpty()) {
            ...
            /***********INSERIRE QUI I DATI PER LA GUI DEGLI ATTACKERS!!!!!!!!!****************/

            GUIAttacker jimmy = new GUIAttacker(potential_attackers);
            Thread t = new Thread(jimmy);
            {
                t.start();
            }

            Scanner creature_list;

            while(jimmy.getInput().isEmpty()){
                try {
                    t.sleep(1);
            ...
            creature_list = new Scanner(jimmy.getInput());
